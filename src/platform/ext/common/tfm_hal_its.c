...
/* The size of flash area emulated in RAM for ITS */
...
#error "ITS_RAM_FS_BLOCK_SIZE must be defined by the target in flash_layout.h"
#endif
...
tfm_hal_its_fs_info(struct tfm_hal_its_fs_info_t *fs_info)
{
...
#elif defined(TFM_HAL_ITS_BLOCK_SIZE)
    /* TFM_HAL_ITS_BLOCK_SIZE - The size of contiguous physical flash logical
...
#elif defined(TFM_HAL_ITS_SECTORS_PER_BLOCK)
    /* TFM_HAL_ITS_SECTORS_PER_BLOCK - The number of contiguous physical flash
     ...
     * TFM_HAL_ITS_SECTORS_PER_BLOCK is supported for CMSIS flash driver only. */
    ARM_DRIVER_FLASH *cmsis_driver = &TFM_HAL_ITS_FLASH_DRIVER;
    fs_info->flash_area_addr = TFM_HAL_ITS_FLASH_AREA_ADDR;
    fs_info->flash_area_size = TFM_HAL_ITS_FLASH_AREA_SIZE;
    fs_info->block_size = TFM_HAL_ITS_SECTORS_PER_BLOCK *
            cmsis_driver->GetInfo()->sector_size;
#else
#error "TFM_HAL_ITS_BLOCK_SIZE or TFM_HAL_ITS_SECTORS_PER_BLOCK must be defined by the target in flash_layout.h"
#endif
