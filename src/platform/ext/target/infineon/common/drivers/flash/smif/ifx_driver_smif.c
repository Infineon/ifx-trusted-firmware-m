...
int32_t ifx_driver_smif_erase_sector(ifx_flash_driver_handler_t handler, uint32_t addr)
{
    ...
    /* IMPROVEMENT: Fix Cy_SMIF_MemEraseSector to avoid alignment of address/size or use other function (DRIVERS-13517) */
    smif_status = Cy_SMIF_MemEraseSector(mem->smif_base,
                                         mem->mem_config,
                                         obj->offset + addr,
                                         obj->flash_info->sector_size,
                                         mem->smif_context);
