...
/* In the case of a flash driver limited to single instance list of arguments of flash driver functions is not modified */
...
/* In the case of a multi image flash driver, flash driver instance is added to the list of arguments of flash driver functions
 * GCC/Clang compilers don't support , ##__VA_ARGS__ syntax, IAR doesn't support __VA_OPT__ syntax */
...
static inline int32_t ifx_driver_flash_initialize(IFX_FDF_ARGS(IFX_DRIVER_FLASH_EVENT cb_event))
{
    ...
    return ifx_flash_driver_initialize(&obj->flash_info, obj->start_address, IFX_DRIVER_FLASH_SECTOR_SIZE);
}
...
static int32_t ifx_driver_flash_program_data(IFX_FDF_ARGS(uint32_t addr, const void *data, uint32_t cnt))
{
    const ifx_driver_flash_obj_t *obj = IFX_DRIVER_FLASH_INSTANCE;
    uint32_t size = cnt * IFX_DRIVER_FLASH_PROGRAM_UNIT;
    int32_t result;
