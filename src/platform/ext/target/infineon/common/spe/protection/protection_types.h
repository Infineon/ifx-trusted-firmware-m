...
#include "protection_ppc.h"
#endif

#define IFX_IS_PARTITION_PRIVILEGED(p_info) (fih_eq((p_info)->ifx_ldinfo->privileged, IFX_FIH_TRUE))
...
/* SPM domain settings - privileged, IFX_PC_TFM_SPM_ID protection context */
#define IFX_PROTECT_SPM_DOMAIN_CFG              (1, IFX_PC_TFM_SPM_ID)

/* PRoT domain settings - IFX_PSA_ROT_PRIVILEGED, IFX_PC_TFM_SPM_ID protection context */
#define IFX_PROTECT_PSA_ROT_DOMAIN_CFG          (IFX_PSA_ROT_PRIVILEGED, IFX_PC_TFM_PROT_ID)

/* ARoT domain settings - IFX_APP_ROT_PRIVILEGED, IFX_PC_TFM_AROT_ID protection context */
#define IFX_PROTECT_APP_ROT_DOMAIN_CFG          (IFX_APP_ROT_PRIVILEGED, IFX_PC_TFM_AROT_ID)

#ifdef TFM_PARTITION_NS_AGENT_TZ
/* NS Agent TZ domain settings - IFX_NS_AGENT_TZ_PRIVILEGED, IFX_NS_AGENT_TZ_PRIVILEGED protection context */
...
#else
#define IFX_GET_PARTITION_PC(p_info, secure) \
        fih_int_encode(IFX_PC_TFM_ID)
#endif

/* Return boundary security domain */
#define IFX_GET_BOUNDARY_DOMAIN(boundary) \
    ((boundary) == IFX_SPM_BOUNDARY ? IFX_PROTECT_SPM_DOMAIN : (((const ifx_partition_info_t *)boundary)->ifx_domain))
...
typedef struct {
     ...
     * \note Set to \ref FIH_INVALID_INIT to disable secure state. */
    fih_int secure_pc;
    /*! \brief Protection Context used to enter non-secure state.
     * \note Set to \ref FIH_INVALID_INIT to disable non-secure state. */
