...
/*
 * Last partition boundary that has been activated. It may not be equal to current partition boundary,
                                              ...
                                              uint32_t asset_cnt)
{
    ...
    for (asset_idx = 0; asset_idx < asset_cnt; asset_idx++) {
        if ((p_assets[asset_idx].attr & ASSET_ATTR_NAMED_MMIO) != 0u) {
             ...
             * ("name": value from enum \ref cy_en_prot_region_fix_t) and
             * ("type": ASSET_ATTR_NAMED_MMIO) */

            /* PPC asset protection */
            /* Named MMIO is not used by platform code, skip protection */
            if (p_assets[asset_idx].dev.dev_ref == PTR_TO_REFERENCE(IFX_UNUSED_MMIO)) {
                continue;
            }
...
#endif /* TFM_ISOLATION_LEVEL == 3) */
            if (fih_eq(ppc_isolation, IFX_FIH_TRUE)) {
                ...
                FIH_CALL(ifx_ppc_isolate_named_mmio, result, &cfg->ppc_cfg, &p_assets[asset_idx]);
                if (fih_not_eq(result, fih_int_encode(TFM_HAL_SUCCESS))) {
                    FIH_RET(result);
                }
            }
     ...
     * saving some memory. Also this improves run time performance */
...
#endif /* TFM_ISOLATION_LEVEL == 3) */
            if (fih_eq(mpc_isolation, IFX_FIH_TRUE)) {
                /* MPC asset protection */
                FIH_CALL(ifx_mpc_isolate_numbered_mmio, result, &cfg->mpc_cfg, &p_assets[asset_idx]);
                if (fih_not_eq(result, fih_int_encode(TFM_HAL_SUCCESS))) {
                    FIH_RET(result);
                }
            }
            ...
            if (fih_eq(cfg->mpu_regions_enable, IFX_FIH_TRUE)) {
                TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_20_7, "Cannot wrap with parentheses due to Fault injection architecture and define FIH_RET_TYPE")
...
FIH_RET_TYPE(enum tfm_hal_status_t) tfm_hal_set_up_static_boundaries(uintptr_t *p_spm_boundary)
{
    ...
    FIH_RET_TYPE(enum tfm_plat_err_t) plat_fih_rc = fih_int_encode(TFM_PLAT_ERR_SYSTEM_ERR);
    *p_spm_boundary = IFX_SPM_BOUNDARY;
    ...
    if (fih_not_eq(plat_fih_rc, fih_int_encode(TFM_PLAT_ERR_SUCCESS))) {
        FIH_RET(fih_int_encode(TFM_HAL_ERROR_GENERIC));
    }

    SPMLOG_DBGMSG("\r\nTF-M setup static boundaries\r\n");

#if IFX_TFM_PROT_RELAXED
    /* Warn that IFX_TFM_PROT_RELAXED is enabled.
     * Warn in both compile and run time.
     * print warning in red color */
    #warning Relaxed protections are applied. This may result in TF-M being non-compliant with certification.
    SPMLOG_INFMSG("\033[1;31m");
    SPMLOG_INFMSG("WARNING: Relaxed protections are applied. This may result in TF-M being non-compliant with certification.");
    SPMLOG_INFMSG("\033[0m\r\n");
#endif
                                           ...
                                           size_t size, uint32_t access_type)
{
    ...
    /* Only TFM_HAL_ACCESS_READABLE, TFM_HAL_ACCESS_WRITABLE and TFM_HAL_ACCESS_NS are used by TFM now */
    ...
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_20_7, "Cannot wrap with parentheses due to Fault injection architecture and define FIH_RET_TYPE")
    FIH_CALL(ifx_tz_memory_check, fih_rc,
             p_info, base, size, access_type);
    if (fih_not_eq(fih_rc, fih_int_encode(TFM_HAL_SUCCESS))) {
        FIH_RET(fih_rc);
    }
    ...
    /* Check MPC access */
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_20_7, "Cannot wrap with parentheses due to Fault injection architecture and define FIH_RET_TYPE")
    FIH_CALL(ifx_mpc_memory_check, fih_rc,
             p_info, base, size, access_type);
    FIH_RET(fih_rc);
}
                                            ...
                                            uintptr_t *p_boundary)
{
    ...
    *p_boundary = (uintptr_t)p_info;
    ...
    if (p_info->ifx_domain == IFX_PROTECT_SPM_DOMAIN) {
        ...
        ifx_asset_protection_type = IFX_ASSET_PROTECTION_PARTITION_DYNAMIC;
        FIH_CALL(ifx_protect_partition_assets, result, p_info, p_info->ifx_ldinfo->protect_cfg_disabled);
        if (fih_not_eq(result, fih_int_encode(TFM_HAL_SUCCESS))) {
            FIH_RET(result);
        }
    }
...
FIH_RET_TYPE(enum tfm_hal_status_t) tfm_hal_finish_boot(void)
{
...
FIH_RET_TYPE(bool) tfm_hal_boundary_need_switch(uintptr_t boundary_from, uintptr_t boundary_to)
{
    ...
    FIH_RET(fih_int_encode(domain_from != domain_to));
    ...
    */
    ...
    if ((boundary != IFX_SPM_BOUNDARY) && (boundary != active_boundary)) {
        ...
        if (IFX_PROTECT_SPM_DOMAIN != old_domain) {
            ...
            FIH_CALL(ifx_protect_partition_assets, result, p_active_info, p_active_info->ifx_ldinfo->protect_cfg_disabled);
            if (fih_not_eq(result, fih_int_encode(TFM_HAL_SUCCESS))) {
                FIH_RET(result);
            }
        }
        ...
        if (IFX_PROTECT_SPM_DOMAIN != new_domain) {
            ...
            result = fih_int_encode(TFM_HAL_SUCCESS);
        }
