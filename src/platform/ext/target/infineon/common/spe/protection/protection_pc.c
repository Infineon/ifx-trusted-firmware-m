 ...
 */
TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_2_8, "TFM-3961, Used with assembler for restoring PC after handling exception");
...
"ifx_arch_switch_protection_context_skip_cache_invalidation:\n"
        ...
        : [ICACHE0_CMD_ADDR] "n" (IFX_NS_ADDRESS_ALIAS((uintptr_t)(&ICACHE0->CMD))),
#else /* IFX_TFM_PROT_RELAXED */
        : [ICACHE0_CMD_ADDR] "n" (&ICACHE0->CMD),
#endif /* IFX_TFM_PROT_RELAXED */
          [PC_ADDR] "n" (&MS_PC0->PC),
 ...
 */
#ifdef FIH_ENABLE_DOUBLE_VARS
__attribute__((naked)) void ifx_arch_switch_protection_context_from_irq(uint32_t pc, uint32_t pc_mask)
#else
