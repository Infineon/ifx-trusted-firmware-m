...
FIH_RET_TYPE(enum tfm_plat_err_t) ifx_faults_cfg(void)
{
    ...
    if ((uint8_t)fault_source != CY_SYSFAULT_NO_FAULT) {
        ...
        SPMLOG_ERRMSG("========================================\r\n");
        SPMLOG_ERRMSG("Platform Exceptions captured on reset...\r\n");
        /* ifx_faults_dump() logs each of Pending Faults and also clears the Fault STATUS for each one */
        ifx_faults_dump();
        SPMLOG_ERRMSG("========================================\r\n");
    }
#else
    ...
    /* Set UsageFault priority to the highest, that is not already set in spm init */
    ...
    for (uint32_t idx = 0UL; idx < ifx_fault_sources_fault_struct0_count; idx++) {
        if (Cy_SysFault_GetMaskByIdx(FAULT_STRUCT0, ifx_fault_sources_fault_struct0[idx]) == false) {
            FIH_RET(fih_int_encode(TFM_PLAT_ERR_SYSTEM_ERR));
        }
    }
...
void tfm_hal_platform_exception_info(void)
{
    ...
    SPMLOG_ERRMSGVAL("Saved Protection Context: ", protection_context);
