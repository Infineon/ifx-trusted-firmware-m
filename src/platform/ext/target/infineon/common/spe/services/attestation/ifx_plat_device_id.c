...
#endif /* defined(CY_DEVICE_CAT1D) */
#include "ifx_s_peripherals_def.h"
#include "tfm_plat_device_id.h"
#include "static_checks.h"

#define IFX_ADD_TO_IMPLEMENTATION_ID(buffer, val)       (void)memcpy((buffer), (const void*)(&(val)), sizeof((val))); \
                                                        (buffer) += sizeof((val));
...
enum tfm_plat_err_t tfm_plat_get_implementation_id(uint32_t *size, uint8_t  *buf)
{
    ...
    uint8_t device_id_to[sizeof(IFX_NON_SECURE_SFLASH_BASE->DEVICE_ID_TO) - IFX_ASSETS_RRAM_CHECKSUM_SIZE];
    uint8_t device_id_mpn[sizeof(IFX_NON_SECURE_SFLASH_BASE->DEVICE_ID_MPN) - IFX_ASSETS_RRAM_CHECKSUM_SIZE];

    if (*size < (sizeof(die_id) + sizeof(device_id_to) + sizeof(device_id_mpn))) {
        return TFM_PLAT_ERR_SYSTEM_ERR;
    }

    status = ifx_assets_rram_read_asset((uint32_t)IFX_NON_SECURE_SFLASH_BASE->DIE_ID, die_id, sizeof(die_id));
    if (status != ARM_DRIVER_OK) {
        return TFM_PLAT_ERR_SYSTEM_ERR;
    }

    status = ifx_assets_rram_read_asset((uint32_t)IFX_NON_SECURE_SFLASH_BASE->DEVICE_ID_TO, device_id_to, sizeof(device_id_to));
    if (status != ARM_DRIVER_OK) {
        return TFM_PLAT_ERR_SYSTEM_ERR;
    }

    status = ifx_assets_rram_read_asset((uint32_t)IFX_NON_SECURE_SFLASH_BASE->DEVICE_ID_MPN, device_id_mpn, sizeof(device_id_mpn));
    if (status != ARM_DRIVER_OK) {
        return TFM_PLAT_ERR_SYSTEM_ERR;
    }
...
#else /* defined(CY_DEVICE_CAT1D) */
#error "Unsupported device category"
#endif /* defined(CY_DEVICE_CAT1D) */
