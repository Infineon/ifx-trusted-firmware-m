...
#define IFX_PLAT_NV_COUNTER_NS_0        (IFX_PLAT_NV_COUNTER_PS_0 + \
                                        ((uint32_t)PLAT_NV_COUNTER_PS_2 - (uint32_t)PLAT_NV_COUNTER_PS_0 + 1U))
...
#if (IFX_PLAT_RRAM_COUNTER_NUMBER > IFX_SE_RRAM_ROLLBACK_COUNTER_NUM_MAX - IFX_SE_RRAM_ROLLBACK_COUNTER_NUM_MIN + 1U)
#error "IFX_PLAT_RRAM_COUNTER_NUMBER is unexpected. SE counters amount lees than expected by TF-M."
#endif
                                               ...
                                               ifx_se_fih_t *num)
{
    ...
    switch (counter_id) {
        ...
        case (PLAT_NV_COUNTER_PS_2):
            mapped_id = IFX_PLAT_NV_COUNTER_PS_0 + (uint32_t)counter_id - (uint32_t)PLAT_NV_COUNTER_PS_0;
            break;
#endif
        ...
        case (PLAT_NV_COUNTER_NS_2):
            mapped_id = IFX_PLAT_NV_COUNTER_NS_0 + (uint32_t)counter_id - (uint32_t)PLAT_NV_COUNTER_NS_0;
            break;
