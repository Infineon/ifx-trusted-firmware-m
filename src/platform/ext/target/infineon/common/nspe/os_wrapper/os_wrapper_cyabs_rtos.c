                            ...
                            uint32_t priority)
{
    ...
    if (CY_RSLT_SUCCESS != cy_rtos_create_thread(&handle, (cy_thread_entry_fn_t)func, name, NULL, ssize,
...
uint32_t os_wrapper_semaphore_acquire(void *handle, uint32_t timeout)
{
    ...
    if (CY_RSLT_SUCCESS != cy_rtos_get_semaphore((cy_semaphore_t *) handle, (cy_time_t) timeout, false))
    {
        return OS_WRAPPER_ERROR;
    }
...
{
    /* IMPROVEMENT: Integrate with cyabs RTOS when MIDDLEWARE-10689 and TFM-3946 are fixed */
    return true;
}
