...
~~~~~~~~~~~~~~~~~~~~~~
Defines the size of contiguous physical flash area that form a logical
...
--------------------
The ``TFM_HAL_ITS_FLASH_AREA_ADDR``, ``TFM_HAL_ITS_FLASH_AREA_SIZE`` and either
``TFM_HAL_ITS_SECTORS_PER_BLOCK`` or ``TFM_HAL_ITS_BLOCK_SIZE`` definitions are
optional. If not defined, the platform must implement ``tfm_hal_its_fs_info()``
instead.

Equivalently, ``tfm_hal_its_ps_info()`` must be implemented by the platform if
``TFM_HAL_ITS_FLASH_AREA_ADDR``, ``TFM_HAL_ITS_FLASH_AREA_SIZE`` and either
``TFM_HAL_ITS_SECTORS_PER_BLOCK`` or ``TFM_HAL_ITS_BLOCK_SIZE`` is not defined.
