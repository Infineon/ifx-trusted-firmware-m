...
- ``TFM_HAL_PS_PROGRAM_UNIT`` - Defines the size of the PS flash device's
  ...
  ``TFM_HAL_PS_FLASH_DRIVER.GetInfo()->program_unit``.
...
- ``TFM_HAL_PS_BLOCK_SIZE`` - Defines the logical block size in the filesystem.
...
- ``PS_RAM_FS_BLOCK_SIZE`` - Defines the size of the RAM FS logical block when
