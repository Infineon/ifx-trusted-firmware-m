...
#if !defined(__DCACHE_PRESENT) || (__DCACHE_PRESENT == 0U) || defined(MAILBOX_IS_UNCACHED_NS)
...
#define MAILBOX_DISABLE_CACHE() SCB_DisableDCache()
#define MAILBOX_ENABLE_CACHE() SCB_EnableDCache()
#define DCACHE_IS_ENABLED (SCB->CCR & SCB_CCR_DC_Msk)
#endif
...
/**
 * \brief Performs multicore boot sequence in NSPE
 *
                                          ...
                                          uint8_t idx)
{
    ...
    /* Avoid problems with PSA parameters that share a cache line with local variables */
    MAILBOX_DISABLE_CACHE();
}

static inline bool all_queue_slots_empty(struct ns_mailbox_queue_t *queue_ptr)
{
    mailbox_queue_status_t empty = ((1UL << (NUM_MAILBOX_QUEUE_SLOT - 1)) - 1)
                                    + (1UL << (NUM_MAILBOX_QUEUE_SLOT - 1));
    return (queue_ptr->empty_slots & empty) == empty;
}
