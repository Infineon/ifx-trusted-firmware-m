...
/* ITS flash driver instance configuration */
static struct its_flash_config_t fs_flash_config_its = {
    .context = ITS_FLASH_DEV,
    .flash_area_addr = 0,
    .block_size = 0,
    .num_blocks = 0,
...
#ifdef TFM_PARTITION_PROTECTED_STORAGE
/* PS flash driver instance configuration */
static struct its_flash_config_t fs_flash_config_ps = {
    .context = PS_FLASH_DEV,
    .flash_area_addr = 0,
    .block_size = 0,
    .num_blocks = 0,
...
static psa_status_t init_its_fs_cfg(void)
{
    ...
    fs_cfg_its.flash_cfg->flash_area_addr = its_fs_info.flash_area_addr;
    fs_cfg_its.flash_cfg->block_size = its_fs_info.block_size;
    if (fs_cfg_its.flash_cfg->block_size == 0) {
        return PSA_ERROR_PROGRAMMER_ERROR;
    }
    fs_cfg_its.flash_cfg->num_blocks = its_fs_info.flash_area_size /
            fs_cfg_its.flash_cfg->block_size;
...
static psa_status_t init_ps_fs_cfg(void)
{
    ...
    /* Derive address, block_size and num_blocks from the HAL parameters */
    fs_cfg_ps.flash_cfg->flash_area_addr = ps_fs_info.flash_area_addr;
    fs_cfg_ps.flash_cfg->block_size = ps_fs_info.block_size;
    if (fs_cfg_ps.flash_cfg->block_size == 0) {
        return PSA_ERROR_PROGRAMMER_ERROR;
    }
    fs_cfg_ps.flash_cfg->num_blocks = ps_fs_info.flash_area_size /
            fs_cfg_ps.flash_cfg->block_size;
...
psa_status_t tfm_its_init(void)
{
    ...
    status = its_flash_fs_init_ctx(&fs_ctx_its, &fs_cfg_its);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    status = its_flash_fs_init_ctx(&fs_ctx_ps, &fs_cfg_ps);
    if (status != PSA_SUCCESS) {
        return status;
    }
