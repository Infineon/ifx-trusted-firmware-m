                                              ...
                                              size_t size)
{
    ...
    if ((lblock != ITS_LOGICAL_DBLOCK0) && (flush_data != 0)) {
        err = fs_ctx->cfg->ops->flush(fs_ctx->cfg->flash_cfg, scratch_id);
    }
                                        ...
                                        uint8_t *buf)
{
    ...
    return fs_ctx->cfg->ops->read(fs_ctx->cfg->flash_cfg, phys_block, buf, pos, size);
}
                                      ...
                                      const uint8_t *data)
{
    ...
    err = fs_ctx->cfg->ops->write(fs_ctx->cfg->flash_cfg, scratch_id, data, pos, size);
    if (err != PSA_SUCCESS) {
        return err;
    }
    ...
    num_bytes = (fs_ctx->cfg->flash_cfg->block_size - block_meta->free_size) - pos;
