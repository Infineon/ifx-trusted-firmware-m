                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
     ...
     */
    return fs_ctx->cfg->flash_cfg->num_blocks == 2 ? 1 : 2;
}
...
static inline uint32_t its_init_dblock_start(struct its_flash_fs_ctx_t *fs_ctx)
{
     ...
     */
    return fs_ctx->cfg->flash_cfg->num_blocks == 2 ? 0 : 3;
}
...
static uint32_t its_num_dedicated_dblocks(struct its_flash_fs_ctx_t *fs_ctx)
{
     ...
     */
     return fs_ctx->cfg->flash_cfg->num_blocks == 2 ? 0 :
            fs_ctx->cfg->flash_cfg->num_blocks - its_init_dblock_start(fs_ctx);
}
                              ...
                              const struct its_metadata_block_header_t *h_meta1)
{
     ...
     */
    if (fs_ctx->cfg->flash_cfg->erase_val == 0x00U) {
        rollover_val = 1;
    } else {
        rollover_val = 0;
    }
                                        ...
                                        const struct its_file_meta_t *file_meta)
{
    ...
    if (its_utils_validate_fid(file_meta->id) == PSA_SUCCESS) {
        ...
        err = its_utils_check_contained_in(fs_ctx->cfg->flash_cfg->block_size,
                                           file_meta->data_idx,
                                           file_meta->max_size);
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_DATA_CORRUPT;
                                      ...
                                      const struct its_block_meta_t *block_meta)
{
    ...
    if (block_meta->phy_id >= fs_ctx->cfg->flash_cfg->num_blocks) {
        return PSA_ERROR_DATA_CORRUPT;
    }
                                              ...
                                              uint8_t *xor_value)
{
    ...
    for (i = 0; i < its_num_active_dblocks(fs_ctx); i++) {
        err = fs_ctx->cfg->ops->read(fs_ctx->cfg->flash_cfg, block_id,
                                metadata,
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
     ...
     */
    err = fs_ctx->cfg->ops->erase(fs_ctx->cfg->flash_cfg, fs_ctx->scratch_metablock);
    if (err != PSA_SUCCESS) {
        return err;
    }
     ...
     */
    if (fs_ctx->cfg->flash_cfg->num_blocks > 2) {
        scratch_datablock =
            its_flash_fs_mblock_cur_data_scratch_id(fs_ctx,
                                                    (ITS_LOGICAL_DBLOCK0 + 1));
        err = fs_ctx->cfg->ops->erase(fs_ctx->cfg->flash_cfg, scratch_datablock);
    }
                                      ...
                                      const struct its_block_meta_t *block_meta)
{
    ...
    return fs_ctx->cfg->ops->write(fs_ctx->cfg->flash_cfg, fs_ctx->scratch_metablock,
                                              ...
                                              uint8_t swap_count)
{
     ...
     */
    return (swap_count == fs_ctx->cfg->flash_cfg->erase_val)
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
     ...
     */
    number = fs_ctx->cfg->flash_cfg->block_size - block_meta_0.free_size -
                            sizeof(struct its_metadata_block_header_comp_t);
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
    ...
    err = fs_ctx->cfg->ops->read(fs_ctx->cfg->flash_cfg, fs_ctx->active_metablock,
                            (uint8_t *)&fs_ctx->meta_block_header, 0,
                            ITS_BLOCK_META_HEADER_SIZE);
    if (err != PSA_SUCCESS) {
        return err;
                                            ...
                                            struct its_block_meta_t *block_meta)
{
    ...
    for (i = 0; i < its_num_active_dblocks(fs_ctx); i++) {
        ...
        if (block_meta->free_size >= size) {
            /* Set file metadata */
            file_meta->lblock = i;
            file_meta->data_idx = fs_ctx->cfg->flash_cfg->block_size
                                  - block_meta->free_size;
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
     ...
     */
    err = fs_ctx->cfg->ops->read(fs_ctx->cfg->flash_cfg, ITS_METADATA_BLOCK0,
    ...
    err = fs_ctx->cfg->ops->read(fs_ctx->cfg->flash_cfg, ITS_METADATA_BLOCK1,
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
    ...
    err = fs_ctx->cfg->ops->flush(fs_ctx->cfg->flash_cfg, fs_ctx->scratch_metablock);
    if (err != PSA_SUCCESS) {
        return err;
    }
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
    ...
    data_size = (fs_ctx->cfg->flash_cfg->block_size - block_meta.data_start)
                - block_meta.free_size;
                                              ...
                                              struct its_flash_fs_ctx_t *fs_ctx)
{
    ...
    err = fs_ctx->cfg->ops->erase(fs_ctx->cfg->flash_cfg, metablock_to_erase_first);
    if (err != PSA_SUCCESS) {
        return err;
    }

    err = fs_ctx->cfg->ops->erase(fs_ctx->cfg->flash_cfg,
    ...
    fs_ctx->meta_block_header.active_swap_count =
                                    (fs_ctx->cfg->flash_cfg->erase_val == 0x00U) ? 1U : 0U;
    ...
    block_meta.data_start =
        its_mblock_file_meta_offset(fs_ctx, fs_ctx->cfg->max_num_files);
    block_meta.free_size = fs_ctx->cfg->flash_cfg->block_size - block_meta.data_start;
    ...
    block_meta.free_size = fs_ctx->cfg->flash_cfg->block_size;
                                              ...
                                              size_t size)
{
    ...
    while (size > 0) {
         ...
         * destination content.
         */
        status = fs_ctx->cfg->ops->read(fs_ctx->cfg->flash_cfg, src_block, dst_block_data_copy,
        ...
        status = fs_ctx->cfg->ops->write(fs_ctx->cfg->flash_cfg, dst_block, dst_block_data_copy,
