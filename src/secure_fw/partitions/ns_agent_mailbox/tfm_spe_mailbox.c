                            ...
                            uint32_t *control)
{
    ...
    for (unsigned int i = 0; i < PSA_MAX_IOVEC; i++) {
        if (i < in_len) {
            vectors[idx].in_vec[i] = params->psa_params.psa_call_params.in_vec[i];
        } else {
    ...
    for (unsigned int i = 0; i < PSA_MAX_IOVEC; i++) {
        if (i < out_len) {
            vectors[idx].out_vec[i] = params->psa_params.psa_call_params.out_vec[i];
        } else {
                                    ...
                                    mailbox_queue_status_t *reply_slots)
{
    ...
    uint32_t control = PARAM_PACK(params->psa_params.psa_call_params.type,
                                  params->psa_params.psa_call_params.in_len,
                                  params->psa_params.psa_call_params.out_len);
    int32_t client_id;
    ...
    case MAILBOX_PSA_VERSION:
        psa_ret = tfm_rpc_psa_version(params->psa_params.psa_version_params.sid);
        sync = true;
        break;
    ...
    case MAILBOX_PSA_CALL:
        ...
        psa_ret = tfm_rpc_psa_call(params->psa_params.psa_call_params.handle,
...
/* Following cases are only needed by connection-based services */
    ...
    case MAILBOX_PSA_CONNECT:
        ...
        psa_ret = tfm_rpc_psa_connect(params->psa_params.psa_connect_params.sid,
    ...
    case MAILBOX_PSA_CLOSE:
        ...
        psa_ret = tfm_rpc_psa_close(params->psa_params.psa_close_params.handle, client_id);
        if (psa_ret != PSA_SUCCESS) {
            sync = true;
        }
        break;
...
#ifndef TFM_NS_MANAGE_NSID
        /* Ignore the client_id passed through the mailbox */
	/* TODO: Use a macro */
        msg_ptr->client_id = -1;
#endif
