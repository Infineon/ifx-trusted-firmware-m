...
#if TFM_NSID_IN_SHARED_DATA
#include "tfm_ns_ctx_shm.h"

extern void set_ns_ctx_mgr(struct tfm_ns_ctx_mgr_t *mgr);
#else
...
static psa_api_svc_func_t psa_api_svc_func_table[] = {
    ...
    (psa_api_svc_func_t)set_ns_ctx_mgr,
};

static uint32_t thread_mode_spm_return(uint32_t result, uint32_t *msp)
{
...
#ifdef TFM_FIH_PROFILE_ON
        (void)fih_delay();
        if (__get_BASEPRI() != SECURE_THREAD_EXECUTION_PRIORITY) {
            tfm_core_panic();
        }
#endif /* TFM_FIH_PROFILE_ON */
    }
...
#ifdef PLATFORM_THREAD_MODE_SPM_RETURN_HOOK
    PLATFORM_THREAD_MODE_SPM_RETURN_HOOK(saved_exc_return, msp);
#endif

    return saved_exc_return;
}

static void init_spm_func_context(psa_api_svc_func_t svc_func, uint32_t *ctx, uint32_t *msp)
{
...
#ifdef PLATFORM_INIT_SPM_FUNC_CONTEXT_HOOK
    PLATFORM_INIT_SPM_FUNC_CONTEXT_HOOK(msp, ctx);
#endif
}
                                          ...
                                          uint32_t *msp)
{
    ...
    init_spm_func_context(svc_func, ctx, msp);
                                        ...
                                        uint32_t *svc_args, uint32_t *msp)
{
    ...
    case TFM_SVC_THREAD_MODE_SPM_RETURN:
        exc_return = thread_mode_spm_return(svc_args[0], msp);
        break;
#endif
    default:
...
uint32_t spm_svc_handler(uint32_t *msp, uint32_t exc_return, uint32_t *psp)
{
    ...
    if (TFM_SVC_IS_PSA_API(svc_number)) {
        ...
        return prepare_to_thread_mode_spm(svc_number, svc_args, exc_return, msp);
    }
#endif
