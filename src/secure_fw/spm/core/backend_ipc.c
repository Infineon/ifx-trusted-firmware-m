 ...
 * Copyright (c) 2021-2025 Cypress Semiconductor Corporation (an Infineon
...
void backend_init_comp_assuredly(struct partition_t *p_pt, uint32_t service_setting)
{
    ...
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_11_6, "Intentional pointer cast to void *")
...
uint64_t ipc_schedule(uint32_t exc_return)
{
    ...
    (void)fih_delay();
    if (__get_BASEPRI() != basepri) {
        tfm_core_panic();
    }
#endif /* TFM_FIH_PROFILE_ON */
#endif

    /* Update meta indicator unconditionally to avoid situation
     * when it's not synchronized with active partition */
    ...
    if (CURRENT_THREAD != &p_part_next->thrd) {
        tfm_core_panic();
    }
