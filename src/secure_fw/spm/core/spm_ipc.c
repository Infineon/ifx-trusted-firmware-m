...
#if CONFIG_TFM_HAL_FINISH_BOOT
    /*
     * Platform can use CONFIG_TFM_HAL_FINISH_BOOT option to add extra initialization
     * steps after static initialization of partitions' isolation has been completed.
     */
    FIH_CALL(tfm_hal_finish_boot, fih_rc);
    if (fih_not_eq(fih_rc, fih_int_encode(TFM_HAL_SUCCESS))) {
        tfm_core_panic();
    }
#endif /* CONFIG_TFM_HAL_FINISH_BOOT */
